...
if __name__ == '__main__':
    ...
    # height = 6
    # width = 9
    # original_walls = [(3, 1), (3, 2), (3, 3), (3, 4), (3, 5), (3, 6), (3, 7), (3, 8)]  # Initial wall positions
    # new_walls = [(3, 1), (3, 2), (3, 3), (3, 4), (3, 5), (3, 6), (3, 7)]  # Walls after change
    # change_time_step = 2000
    # start_position = (5, 3)
    # goal_position = (0, 8)
    # total_timesteps = 100000  # Set the total timesteps limit

    # Simpler Maze setup for debug
    height = 3
    width = 3
    original_walls = [(1, 1), (1, 2)]  # Initial wall positions
    new_walls = [(1, 1)]  # Walls after change
    change_time_step = 200
    start_position = (2, 1)
    goal_position = (0, 2)
    total_timesteps = 10000  # Set the total timesteps limit

    print("The Shortcut Maze environment")
    env = DynamicMaze(
        height=height, 
        width=width, 
    ...
    alpha = 0.1
    agents = {
        "DynaQ": DynaQWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5),
        "DynaQ+ kappa=1e-4": DynaQPlusWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5, kappa=1e-4),
        "DynaQ+ kappa=1e-3": DynaQPlusWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5, kappa=1e-3),
        "DynaQ+ kappa=1e-2": DynaQPlusWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5, kappa=1e-2),
        "DynaQ+ kappa=1e-1": DynaQPlusWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5, kappa=1e-1),
        "DynaQ+ kappa=1e-0": DynaQPlusWithLogging(env=env, gamma=gamma, epsilon=epsilon, alpha=alpha, planning_steps=5, kappa=1.0),
    }
    ...
    for agent_name, agent in agents.items():
        ...
        with tqdm(total=num_runs, desc=f"{agent_name}") as pbar:
            for _ in range(num_runs):
                ...
                # env._print_policy(agent.policy)
    ...
    plt.title('Cumulative Rewards vs. Total Timesteps for DynaQ and DynaQPlus in Shortcut Maze')
    plt.legend()
    plt.grid(True)

    # Save and show the plot
    plot_path = os.path.join(log_dir, 'shortcut_maze_avg_cumulative_rewards_vs_timesteps.png')
